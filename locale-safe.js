(() => {
  ...
  const UI = {
    ko:{lang:'ko',all:'전체',old:'구약',new:'신약',chapter:n=>`${n}장`,verse:n=>`${n}절`,search:'검색',compare:'비교',prev:'← 이전 장',next:'다음 장 →',single:'한 면 보기',dual:'양면 보기',placeholder:'검색',loading:'본문을 불러오는 중…',title:(b,c)=>`${b} ${c}장`,book:i=>BOOKS[i]?.ko||''},
    en:{lang:'en',all:'All',old:'Old Testament',new:'New Testament',chapter:n=>`Chapter ${n}`,verse:n=>`Verse ${n}`,search:'Search',compare:'Compare',prev:'← Previous',next:'Next →',single:'Single view',dual:'Two-page view',placeholder:'Search',loading:'Loading Scripture…',title:(b,c)=>`${b} ${c}`,book:i=>EN_BOOKS[i]||''},
    fr:{lang:'fr',all:'Tout',old:'Ancien Testament',new:'Nouveau Testament',chapter:n=>`Chapitre ${n}`,verse:n=>`Verset ${n}`,search:'Rechercher',compare:'Comparer',prev:'← Précédent',next:'Suivant →',single:'Vue simple',dual:'Double page',placeholder:'Rechercher',loading:'Chargement…',title:(b,c)=>`${b} ${c}`,book:i=>EN_BOOKS[i]||''},
    de:{lang:'de',all:'Alle',old:'Altes Testament',new:'Neues Testament',chapter:n=>`Kapitel ${n}`,verse:n=>`Vers ${n}`,search:'Suchen',compare:'Vergleichen',prev:'← Zurück',next:'Weiter →',single:'Einzelseite',dual:'Doppelseite',placeholder:'Suchen',loading:'Bibeltext wird geladen…',title:(b,c)=>`${b} ${c}`,book:i=>EN_BOOKS[i]||''},
    zh:{lang:'zh',all:'全部',old:'旧约',new:'新约',chapter:n=>`第${n}章`,verse:n=>`第${n}节`,search:'搜索',compare:'对照',prev:'← 上一章',next:'下一章 →',single:'单页',dual:'双页',placeholder:'搜索',loading:'正在载入经文…',title:(b,c)=>`${b} 第${c}章`,book:i=>EN_BOOKS[i]||''},
    ru:{lang:'ru',all:'Все',old:'Ветхий Завет',new:'Новый Завет',chapter:n=>`Глава ${n}`,verse:n=>`Стих ${n}`,search:'Поиск',compare:'Сравнить',prev:'← Назад',next:'Далее →',single:'Одна страница',dual:'Две страницы',placeholder:'Поиск',loading:'Загрузка текста…',title:(b,c)=>`${b} ${c}`,book:i=>EN_BOOKS[i]||''},
    la:{lang:'la',all:'Omnia',old:'Vetus Testamentum',new:'Novum Testamentum',chapter:n=>`Caput ${n}`,verse:n=>`Versus ${n}`,search:'Quaere',compare:'Compara',prev:'← Prior',next:'Proximus →',single:'Una pagina',dual:'Duae paginae',placeholder:'Quaere',loading:'Textus oneratur…',title:(b,c)=>`${b} ${c}`,book:i=>EN_BOOKS[i]||''}
  };
