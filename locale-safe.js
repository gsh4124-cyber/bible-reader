(() => {
  ...
  const UI = {
    ko:{lang:'ko',all:'전체',old:'구약',new:'신약',chapter:n=>`${n}장`,verse:n=>`${n}절`,search:'검색',compare:'비교',prev:'← 이전 장',next:'다음 장 →',single:'한 면 보기',dual:'양면 보기',placeholder:'책 이름 · 단어 · 요 3:16',dailyVerse:'오늘의 성구',dailyChapter:'오늘의 묵상장',open:'열기 →',loading:'본문을 불러오는 중…',title:(b,c)=>`${b} ${c}장`,book:i=>BOOKS[i]?.ko||''},
    en:{lang:'en',all:'All',old:'Old Testament',new:'New Testament',chapter:n=>`Chapter ${n}`,verse:n=>`Verse ${n}`,search:'Search',compare:'Compare',prev:'← Previous',next:'Next →',single:'Single view',dual:'Two-page view',placeholder:'Book · keyword · John 3:16',dailyVerse:'Verse of the day',dailyChapter:'Today’s meditation',open:'Open →',loading:'Loading Scripture…',title:(b,c)=>`${b} ${c}`,book:i=>EN_BOOKS[i]||''},
    fr:{lang:'fr',all:'Tout',old:'Ancien Testament',new:'Nouveau Testament',chapter:n=>`Chapitre ${n}`,verse:n=>`Verset ${n}`,search:'Rechercher',compare:'Comparer',prev:'← Précédent',next:'Suivant →',single:'Vue simple',dual:'Double page',placeholder:'Livre · mot · Jean 3:16',dailyVerse:'Verset du jour',dailyChapter:'Méditation du jour',open:'Ouvrir →',loading:'Chargement…',title:(b,c)=>`${b} ${c}`,book:i=>EN_BOOKS[i]||''},
    de:{lang:'de',all:'Alle',old:'Altes Testament',new:'Neues Testament',chapter:n=>`Kapitel ${n}`,verse:n=>`Vers ${n}`,search:'Suchen',compare:'Vergleichen',prev:'← Zurück',next:'Weiter →',single:'Einzelseite',dual:'Doppelseite',placeholder:'Buch · Wort · Joh 3,16',dailyVerse:'Vers des Tages',dailyChapter:'Andacht des Tages',open:'Öffnen →',loading:'Bibeltext wird geladen…',title:(b,c)=>`${b} ${c}`,book:i=>EN_BOOKS[i]||''},
    zh:{lang:'zh',all:'全部',old:'旧约',new:'新约',chapter:n=>`第${n}章`,verse:n=>`第${n}节`,search:'搜索',compare:'对照',prev:'← 上一章',next:'下一章 →',single:'单页',dual:'双页',placeholder:'书卷 · 关键词 · 约 3:16',dailyVerse:'今日经文',dailyChapter:'今日默想',open:'打开 →',loading:'正在载入经文…',title:(b,c)=>`${b} 第${c}章`,book:i=>EN_BOOKS[i]||''},
    ru:{lang:'ru',all:'Все',old:'Ветхий Завет',new:'Новый Завет',chapter:n=>`Глава ${n}`,verse:n=>`Стих ${n}`,search:'Поиск',compare:'Сравнить',prev:'← Назад',next:'Далее →',single:'Одна страница',dual:'Две страницы',placeholder:'Книга · слово · Ин 3:16',dailyVerse:'Стих дня',dailyChapter:'Размышление дня',open:'Открыть →',loading:'Загрузка текста…',title:(b,c)=>`${b} ${c}`,book:i=>EN_BOOKS[i]||''},
    la:{lang:'la',all:'Omnia',old:'Vetus Testamentum',new:'Novum Testamentum',chapter:n=>`Caput ${n}`,verse:n=>`Versus ${n}`,search:'Quaere',compare:'Compara',prev:'← Prior',next:'Proximus →',single:'Una pagina',dual:'Duae paginae',placeholder:'Liber · verbum · Io 3:16',dailyVerse:'Versus diei',dailyChapter:'Meditatio diei',open:'Aperi →',loading:'Textus oneratur…',title:(b,c)=>`${b} ${c}`,book:i=>EN_BOOKS[i]||''}
  };
  ...
  function applyTitle() {
    ...
    if(title) title.textContent=t().title(t().book(state.bookIndex),state.chapter);
    document.title=`${t().title(t().book(state.bookIndex),state.chapter)} · ${(TRANSLATIONS[translation.value]||TRANSLATIONS.krv1961).name}`;
  }
  ...
  function schedule(){ [0,80,220,500,1000,1800].forEach(ms=>setTimeout(applyAll,ms)); }
