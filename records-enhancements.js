(() => {
  ...
  function goToChapter(item){
    ...
    if (typeof window.loadCurrent === 'function') window.loadCurrent({scrollTop:true});
  }
  ...
  function renderChapters(panel){
    ...
    list.innerHTML = '';
    if (!items.length) {
      list.innerHTML = '<p class="notebook-empty">저장한 장이 없습니다.</p>';
      return;
    }
    items.forEach(item => {
      const card = document.createElement('article');
      card.className = 'notebook-item chapter-record-item';
      const title = document.createElement('strong');
      title.textContent = item.label || `${window.BOOKS?.[item.bookIndex]?.ko || ''} ${item.chapter}장`;
      const actions = document.createElement('div');
      actions.className = 'notebook-item-actions';
      const go = document.createElement('button');
      go.type = 'button'; go.textContent = '장으로 이동';
      go.addEventListener('click', () => goToChapter(item));
  ...
  function exportData(){
    const payload = {
      app:'bible-reader', version:BACKUP_VERSION, exportedAt:new Date().toISOString(),
      verseMarks:readJson(MARKS_KEY), chapterBookmarks:readJson(CHAPTERS_KEY)
    };
    const blob = new Blob([JSON.stringify(payload,null,2)], {type:'application/json'});
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `bible-reader-backup-${new Date().toISOString().slice(0,10)}.json`;
  ...
  function enhance(panel){
    ...
    chapterTab.addEventListener('click', () => {
      tabs.querySelectorAll('button').forEach(b=>b.classList.toggle('active', b===chapterTab));
      renderChapters(panel);
    });
    tabs.append(chapterTab);

    tabs.querySelectorAll('button:not([data-tab="chapters"])').forEach(button => {
      button.addEventListener('click', () => chapterTab.classList.remove('active'));
    });
  ...
  enhance(document.querySelector('.notebook-panel'));
})();
